...
import verifyRule from '../crossTableCheck/verifyRule';
...
provide('groupType', 'newMultiClusterRule');
provide('ruleType', '3-3');
// 组件ref
const ele = ref({
...
useListener(saveEvent, async (cb) => {
    try {
        ...
        const body = {
            ...
            right_linkis_udf_names,
            // eslint-disable-next-line camelcase
            template_arguments: [TEMPLATE_ID.CUSTOM_SQL_CONSISTENCY_VALIDATION, TEMPLATE_ID.CUSTOM_SQL_CONSISTENCY_VALIDATION1].includes(rule_template_name) ? buildCustomSqlTemp(leftArguments, rightArguments, ruleArgumentList) : (ruleArgumentList || []).map(({
                argument_step,
                argument_id,
                argument_value,
                argsSelectList,
