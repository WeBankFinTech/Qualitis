<template>
    <div>
        <micro-app v-if="url" class="microAppClass" name="flinkPoject" :url="url" :data="baseUrlData" default-page="#/app-manage/application/flink"></micro-app>
    </div>
</template>
<script setup>
...
async function initMicroApp() {
    try {
        ...
        const data = await request(`/mfgov/fesdk/bindQuery/v2?${generateUrlTail()}`, {
            main_mf_env: 'prd',
            sub_mf_env: 'prd',
            main_mf_name: 'dqm_realtime_proj_flink_jar_manage',
            sub_mf_name: 'rcs_realtime_flink_jar_config',
        }, {
            method: 'get',
            baseURL: microAppUrl,
            headers: {
...
microApplication.addDataListener('flinkPoject', (data) => {
    const { name, datagroupId } = data;
    if (name === 'realtime-data') {
        router.push({
            path: '/dataSourceManagement/realTimeDataSource',
            query: {
                datagroupId,
            },
        });
