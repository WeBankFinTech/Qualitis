<template>
    <div style="padding: 16px 16px;">
        <micro-app v-if="url" class="microAppClass" name="realTimeDataSource" :url="url" :data="baseUrlData" default-page="#/datasource-manage/realtime-data"></micro-app>
    </div>
</template>
<script setup>
import {
...
const datagroupId = computed(() => route.query.datagroupId);
...
async function initMicroApp() {
    try {
        ...
        const data = await request(`/mfgov/fesdk/bindQuery/v2?${generateUrlTail()}`, {
            main_mf_env: 'prd',
            sub_mf_env: 'prd',
            main_mf_name: 'dqm_realtime_datasource_manage',
            sub_mf_name: 'rcs_realtime_datasource_config',
        }, {
            method: 'get',
            baseURL: microAppUrl,
            headers: {
        ...
        baseUrlData.value.datagroupId = datagroupId.value;
