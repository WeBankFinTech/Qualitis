<template>
    <div style="padding: 16px 16px;">
        <micro-app v-if="url" class="microAppClass" name="realTimeTasks" :url="url" :data="baseUrlData" default-page="#/operations-manager/applicationJob-list"></micro-app>
    </div>
</template>
<script setup>
...
async function initMicroApp() {
    try {
        ...
        const data = await request(`/mfgov/fesdk/bindQuery/v2?${generateUrlTail()}`, {
            main_mf_env: 'prd',
            sub_mf_env: 'prd',
            main_mf_name: 'dqm_realtime_task_manage',
            sub_mf_name: 'rcs_realtime_task_manage',
        }, {
            method: 'get',
            baseURL: microAppUrl,
            headers: {
