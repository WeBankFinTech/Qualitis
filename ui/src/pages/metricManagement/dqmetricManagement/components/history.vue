        ...
        <div v-if="type === 'table' && columns.length" class="table">
            <div v-for="(table) in columns" :key="table.rule_metric_id" class="wrap">
                ...
                <f-table :data="table.rule_metric_values">
                    ...
                    <f-table-column prop="generate_time" :label="$t('_.生成时间')" :width="184" ellipsis></f-table-column>
                    <f-table-column prop="datasource_names" :label="$t('_.数据源名')" :width="184" ellipsis>
                        <template #default="{ row }">{{row.datasource_names?.join('、') || '--'}}</template>
                    </f-table-column>
                    <f-table-column prop="env_name" :label="$t('_.环境名')" :width="184" ellipsis>
                        <template #default="{ row }">{{row?.env_name || '--'}}</template>
                    </f-table-column>

                    <f-table-column #default="{ row = {}}" prop="related_rule_name" :label="$t('_.关联规则')" :width="160" ellipsis>
...
import { getHistory, batchHistory } from '@/pages/metricManagement/dqmetricManagement/api';
