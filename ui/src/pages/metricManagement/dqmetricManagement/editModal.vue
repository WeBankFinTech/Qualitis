<template>
    <FDrawer
        ...
        <FForm v-show="mode === FORM_MODE.ADD || mode === FORM_MODE.EDIT" ref="formRef" label-width="72px" class="metric-form" :model="formModel" :rules="formRule">
            ...
            <!-- 计算模式 -->
            <FFormItem :label="$t('indexManagement.calculationMode')" prop="calculation_mode">
                <FSelect v-model="formModel.calculation_mode" clearable filterable valueField="value" labelField="label" :options="calculations">
                </FSelect>
            </FFormItem>
            <!-- 监控能力 -->
            <FFormItem :label="$t('indexManagement.monitoringCapabilities')" prop="monitoring_capabilities">
                <FSelect v-model="formModel.monitoring_capabilities" clearable filterable tag multiple valueField="value" labelField="label" :options="monitoringCapabilitiesList">
                </FSelect>
            </FFormItem>
            <!-- 指标口径 -->
            <FFormItem :label="$t('indexManagement.metricDefinition')" prop="metric_definition">
                <FInput v-model="formModel.metric_definition" :placeholder="$t('common.pleaseEnter')" :maxlength="200" type="textarea" showWordLimit />
            </FFormItem>
            <!-- 业务域 -->
            <FFormItem :label="$t('indexManagement.businessDomain')" prop="business_domain">
                <FSelect v-model="formModel.business_domain" clearable filterable valueField="value" labelField="label" :options="businessDomains">
                </FSelect>
            </FFormItem>
            <!-- 业务策略 -->
            <FFormItem :label="$t('indexManagement.businessStrategy')" prop="business_strategy">
                <FInput v-model="formModel.business_strategy" :maxlength="200" :placeholder="$t('common.pleaseEnter')" type="textarea" showWordLimit />
            </FFormItem>
            <!-- 业务系统 -->
            <FFormItem :label="$t('indexManagement.businessSystem')" prop="business_system">
                <FInput v-model="formModel.business_system" :maxlength="200" :placeholder="$t('common.pleaseEnter')" type="textarea" showWordLimit />
            </FFormItem>
            <!-- 业务模型 -->
            <FFormItem :label="$t('indexManagement.businessModel')" prop="business_model">
                <FInput v-model="formModel.business_model" :maxlength="200" :placeholder="$t('common.pleaseEnter')" type="textarea" showWordLimit />
            </FFormItem>
        </FForm>
        <template #footer>
...
import {
    defineProps, defineEmits, reactive, ref, toRefs, computed, defineExpose,
} from 'vue';
...
const monitoringCapabilitiesList = ref([]);
const { t: $t } = useI18n();
...
// 弹窗title
const titleMapping = {
    [FORM_MODE.ADD]: $t('_.新增指标'),
    [FORM_MODE.EDIT]: $t('_.指标编辑'),
    [FORM_MODE.REVIEW]: $t('_.指标详情'),
};
...
const handleDeleteMetric = () => {
    ...
    if (!formModel.is_editable) return FMessage.error($t('_.没有权限删除'));
...
const getBusinessDomain = async () => {
    try {
        const res = await fetchBusinessDomain();
        businessDomains.value = res.map(item => ({ value: item.appdomain_cnname, label: item.appdomain_cnname }));
    } catch (err) {
        console.warn(err);
...
// 获取监控能力
const getMonitoringCapabilities = async () => {
    try {
        const res = await fetchMonitoringCapabilities();
        monitoringCapabilitiesList.value = res.map(item => ({ value: item, label: item }));
    } catch (err) {
        console.warn(err);
        monitoringCapabilitiesList.value = [];
    }
};
...
async function init(payload, modeType) {
