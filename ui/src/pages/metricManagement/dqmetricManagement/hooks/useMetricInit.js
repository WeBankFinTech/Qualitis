...
import { fetchOptions, fetchEnCodes } from '../api';
...
export default function useMetricInit() {
    ...
    const handleSearchEnCodes = async (encode = '') => {
        const enCodeRes = await fetchEnCodes(encode);
        enCodes.value = enCodeRes.map(item => ({
            label: item,
            value: item,
        })) || [];
    };
