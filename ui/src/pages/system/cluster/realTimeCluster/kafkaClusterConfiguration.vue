<template>
    <div>
        <micro-app v-if="url" class="microAppClass" name="kafkaClusterConfiguration" :url="url" :data="baseUrlData" default-page="#/env-manage/kafka-env/brokers"></micro-app>
    </div>
</template>
<script setup>
import { ref, onMounted } from 'vue';
import { request, useRouter } from '@fesjs/fes';
import microApplication from '@micro-zoe/micro-app';
...
async function initMicroApp() {
    try {
        ...
        const data = await request(`/mfgov/fesdk/bindQuery/v2?${generateUrlTail()}`, {
            main_mf_env: 'prd',
            sub_mf_env: 'prd',
            main_mf_name: 'dqm_realtime_kafka_cluster_manage',
            sub_mf_name: 'rcs_realtime_kafka_cluster_config',
        }, {
            method: 'get',
            baseURL: microAppUrl,
            headers: {
...
microApplication.addDataListener('kafkaClusterConfiguration', (data) => {
    const { name, clusterId, clusterName } = data;
    if (name === 'kafka-cluster') {
        router.push({
            path: '/system/cluster/realTimeCluster/kafkaTopicConfiguration',
            query: {
                clusterId,
                clusterName,
            },
