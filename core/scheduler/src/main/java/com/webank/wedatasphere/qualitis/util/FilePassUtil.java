 ...
 * @author allenzhou
 */
public class FilePassUtil {

    private static final Logger LOGGER = LoggerFactory.getLogger(FilePassUtil.class);

    private FilePassUtil() {
        // Default Constructor
    }

    public static Boolean pass(TaskRuleAlarmConfig alarmConfig, TaskResult taskResult, TaskResultDao taskResultDao) {
        ...
        if (checkTemplate.equals(CheckTemplateEnum.MONTH_FLUCTUATION.getCode())) {
            ...
            thresholds /= 100;
            try {
                result = getRingGrowth(taskResultDao, checkTemplate, taskResult.getRuleId(), taskResult.getRuleMetricId(), taskResult.getApplicationId());
    ...
    private static Double getAvg(Date nowDate, TaskResultDao taskResultDao, Integer calendarStepUnit, Long ruleId, Long ruleMetricId, String applicationId) {
        ...
        return taskResultDao.findAvgByCreateTimeBetweenAndRuleAndMetricAndApplication(QualitisConstants.PRINT_TIME_FORMAT.format(lastMonthDate), QualitisConstants.PRINT_TIME_FORMAT.format(nowDate), ruleId, ruleMetricId, applicationId);
    }
    ...
    private static long countValue(Date nowDate, TaskResultDao taskResultDao, Integer calendarStepUnit, Long ruleId, Long ruleMetricId, String applicationId) {
        ...
        return taskResultDao.countByCreateTimeBetweenAndRuleAndMetricAndApplication(QualitisConstants.PRINT_TIME_FORMAT.format(lastMonthDate), QualitisConstants.PRINT_TIME_FORMAT.format(nowDate), ruleId, ruleMetricId, applicationId);
    }

    private static Double getRingGrowth(TaskResultDao taskResultDao, Integer ringType, Long ruleId, Long ruleMetricId, String applicationId) throws ArgumentException {
        ...
        if (ringType.equals(CheckTemplateEnum.FULL_YEAR_RING_GROWTH.getCode())) {
            ...
            return specialTimeRingGrowth(start, end, startOfLast, taskResultDao, ruleId, ruleMetricId, applicationId);
    ...
    private static Double specialTimeRingGrowth(LocalDateTime start, LocalDateTime end, LocalDateTime startOfLast, TaskResultDao taskResultDao,
                                                Long ruleId
            , Long ruleMetricId, String applicationId) throws ArgumentException {
        Date startDate;
        Date endDate;
        ...
        Double avgOfYear = taskResultDao.findAvgByCreateTimeBetweenAndRuleAndRuleMetric(QualitisConstants.PRINT_TIME_FORMAT.format(startDate)
                , QualitisConstants.PRINT_TIME_FORMAT.format(endDate), ruleId, ruleMetricId);
        // Calculate pre time area.
        ...
        if (recordNumber > 0) {
            Double avgOfLastYear = taskResultDao.findAvgByCreateTimeBetweenAndRuleAndRuleMetric(QualitisConstants.PRINT_TIME_FORMAT.format(startOfLastDate)
