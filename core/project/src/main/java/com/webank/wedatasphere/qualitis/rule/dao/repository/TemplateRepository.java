...
public interface TemplateRepository extends JpaRepository<Template, Long>, JpaSpecificationExecutor<Template> {
     ...
     * @param tableDataType   要查询的数据权限类型
     * @param dataVisibilityDeptList  用户自身所属的科室可见范围
     * @param createUserId   查询人自身的用户ID，以便能查到自己创建的模板记录
     * @param cnName
     * @param enName
     * @param verificationLevel
     * @param verificationType
     * @param createId    要查询的目标创建人
     * @param modifyId    要查询的模板修改人
     * @param devDepartmentId    要查询的目标开发科室
     * @param opsDepartmentId    要查询的目标运维科室
     * @param actionRange
     * @param pageable
     * @return
     */
    @Query(value = "SELECT qt.* FROM qualitis_template qt " +
            ...
            " AND (?15 is null or qt.create_time >= ?15)" +
            " AND (?16 is null or qt.create_time <= ?16)" +
            " AND (?17 is null or qt.modify_time >= ?17)" +
            " AND (?18 is null or qt.modify_time <= ?18)" +
            " AND (?19 is null or qt.id = ?19)" +
            " AND (?20 is null or qt.description like CONCAT('%', CONCAT(?20, '%') ))"
    ...
    Page<Template> findTemplates(Integer type, Integer dataSourceTypeId
            , String tableDataType, List<Long> dataVisibilityDeptList, Long createUserId, String cnName, String enName, Long verificationLevel
            , Long verificationType, String createId, String modifyId, Long devDepartmentId, Long opsDepartmentId, Set<String> actionRange
    ...
    @Query(value = "SELECT qt.* FROM qualitis_template qt " +
            " where (?1 is null or qt.template_type = ?1) " +
            " AND qt.id in(SELECT tdst.template_id FROM qualitis_template_datasource_type tdst where tdst.template_id=qt.id AND (?2 is null or tdst.data_source_type_id = ?2) ) " +
            " AND if(nullif(?4,'')!='',qt.name like ?4,1=1) " +
            " AND if(nullif(?5,'')!='',qt.en_name like ?5,1=1) " +
            " AND (?6 is null or qt.verification_level = ?6) " +
            " AND (?7 is null or qt.verification_type = ?7) " +
            " AND (?8 is null or qt.create_user_id = ?8) " +
            " AND (?9 is null or qt.modify_user_id = ?9) " +
            " AND (?10 is null or qt.dev_department_id = ?10) " +
            " AND (?11 is null or qt.ops_department_id = ?11) " +
            " AND (coalesce(?12,null) is null or exists (select q.table_data_id from qualitis_auth_data_visibility as q where q.table_data_id=qt.id and q.table_data_type=?3 and (q.department_sub_id in (?12) or q.department_sub_id=0) )) " +
            " AND (?13 is null or qt.create_time >= ?13)" +
            " AND (?14 is null or qt.create_time <= ?14)" +
            " AND (?15 is null or qt.modify_time >= ?15)" +
            " AND (?16 is null or qt.modify_time <= ?16)" +
            " AND (?17 is null or qt.id = ?17)" +
            " AND (?18 is null or qt.description like CONCAT('%', CONCAT(?18, '%') ))"
            , countQuery = "SELECT count(*) FROM qualitis_template qt " +
            " where (?1 is null or qt.template_type = ?1) " +
            " AND qt.id in(SELECT tdst.template_id FROM qualitis_template_datasource_type tdst where tdst.template_id=qt.id AND (?2 is null or tdst.data_source_type_id = ?2) ) " +
            " AND if(nullif(?4,'')!='',qt.name like ?4,1=1) " +
            ...
            " AND (?18 is null or qt.description like CONCAT('%', CONCAT(?18, '%') ))"
            , nativeQuery = true)
    Page<Template> findTemplatesWithAdmin(Integer type, Integer dataSourceTypeId, String tableDataType
            , String cnName, String enName, Long verificationLevel
            , Long verificationType, String createId, String modifyId
            , Long devDepartmentId, Long opsDepartmentId, Set<String> actionRange
            , String createStartTime, String createEndTime, String updateStartTime
            , String updateEndTime, Long templateId, String description
            , Pageable pageable);
    ...
    /**
     * find All Templates By ProjectId
     *
     * @param projectId
     * @return
     */
    @Query(value = "select r.template_id as template_id, r.rule_template_name as template_name from qualitis_rule r where r.project_id =?1 group by template_id", nativeQuery = true)
     ...
     * @return
     */
    @Query(value = "select r.id as template_id, r.name as template_name from qualitis_template r where r.template_type in(?1) " +
            " AND r.id in(SELECT tdst.template_id FROM qualitis_template_datasource_type tdst where tdst.template_id=r.id AND tdst.data_source_type_id = ?2) ", nativeQuery = true)
    List<Map<String, Object>> findHiveDataSourceTypeAllTemplates(List<Integer> dataSourceTypes, Integer code);

    /**
     * get Template Default Input Meta
     *
     * @param enNames
     * @return
     */
    @Query(value = "select * from qualitis_template_default_input_meta where en_name in (?1)"
            , nativeQuery = true)
    ...
    /**
     * query templates by calcu_unit_id
     *
     * @param calcuUnitIds
     * @return
     */
    List<Template> findByCalcuUnitIdIn(List<Long> calcuUnitIds);
}
