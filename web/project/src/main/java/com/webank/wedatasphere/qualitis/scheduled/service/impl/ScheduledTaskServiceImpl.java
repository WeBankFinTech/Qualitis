...
//    private ScheduledWorkflowBusinessDao scheduledWorkflowBusinessDao;
    ...
    public GeneralResponse<Object> frontAndBackRuleGroup(ModifyRuleGroupRequest request) throws UnExpectedRequestException, PermissionDeniedRequestException {
        ...
        if (scheduledTask.getId().toString().equals(scheduledFrontBackRule.getScheduledTask().getId().toString())) {
            ...
            List<ScheduledFrontBackRule> frontRule = goodVos.stream().filter(x -> WtssCommonConstants.WTSS_FRONT_TYPE.equals(x.getTriggerType().toString())).collect(Collectors.toList());
            List<ScheduledFrontBackRule> backRule = goodVos.stream().filter(x -> WtssCommonConstants.WTSS_BACK_TYPE.equals(x.getTriggerType().toString())).collect(Collectors.toList());
            ...
            if (WtssCommonConstants.WTSS_STATUS_CODE.equals(stringObjectMap.get(WtssCommonConstants.WTSS_RESPONSE_CODE).toString())) {
                ...
                List<ScheduledFrontBackRule> frontGroup = queryScheduledTask.stream().filter(x -> WtssCommonConstants.WTSS_FRONT_TYPE.equals(x.getTriggerType().toString())).collect(Collectors.toList());
                List<Long> frontGroupCollect = frontGroup.stream().map(x -> x.getRuleGroup().getId()).collect(Collectors.toList());
                List<ScheduledFrontBackRule> backGroup = queryScheduledTask.stream().filter(x -> WtssCommonConstants.WTSS_BACK_TYPE.equals(x.getTriggerType().toString())).collect(Collectors.toList());
                List<Long> backGroupCollect = backGroup.stream().map(x -> x.getRuleGroup().getId()).collect(Collectors.toList());

                if (WtssCommonConstants.WTSS_FRONT_TYPE.equals(scheduledFrontBackRule.getTriggerType().toString())) {
                    frontGroupCollect.add(scheduledFrontBackRule.getRuleGroup().getId());
                } else if (WtssCommonConstants.WTSS_BACK_TYPE.equals(scheduledFrontBackRule.getTriggerType().toString())) {
    ...
    public GeneralResponse<Object> deleteScheduledTask(DeleteScheduledRequest request) throws UnExpectedRequestException, PermissionDeniedRequestException {
        ...
        List<ScheduledFrontBackRule> frontRule = byScheduledTask.stream().filter(x -> WtssCommonConstants.WTSS_FRONT_TYPE.equals(x.getTriggerType().toString())).collect(Collectors.toList());
        List<Long> frontCollect = frontRule.stream().map(x -> x.getId()).collect(Collectors.toList());
        List<ScheduledFrontBackRule> backRule = byScheduledTask.stream().filter(x -> WtssCommonConstants.WTSS_BACK_TYPE.equals(x.getTriggerType().toString())).collect(Collectors.toList());
...
//        判断是否有必要同步远端删除
        ClusterInfo clusterInfo = clusterInfoDao.findByClusterName(scheduledTask.getClusterName());
        if (clusterInfo == null) {
            request.setClearLocalData(Boolean.TRUE);
        }

        ScheduledOperateHistory scheduledOperateHistory = new ScheduledOperateHistory();
        try {
            if (Boolean.TRUE.equals(request.getClearLocalData())) {
                ...
                deleteScheduledTaskInDb(scheduledFrontBackRule, scheduledTask, collect);
                return new GeneralResponse<>(ResponseStatusConstants.OK, "{&ASSOCIATED_SCHEDULING_TO_WTSS_SUCCESSFULLY}", null);
            } else {
                ...
                if (WtssCommonConstants.WTSS_STATUS_CODE.equals(stringObjectMap.get(WtssCommonConstants.WTSS_RESPONSE_CODE).toString())) {
                    ...
                    return new GeneralResponse<>(stringObjectMap.getOrDefault(WtssCommonConstants.WTSS_RESPONSE_CODE, ResponseStatusConstants.SERVER_ERROR).toString(), stringObjectMap.get(WtssCommonConstants.WTSS_RES_ERROR).toString(), null);
                }
            }
    ...
    private void deleteScheduledTaskInDb(ScheduledFrontBackRule scheduledFrontBackRule, ScheduledTask scheduledTask, List<ScheduledFrontBackRule> byScheduledTask) {
        if (CollectionUtils.isEmpty(byScheduledTask)) {
    ...
    public void deleteRelationScheduleTasks(String projectName) throws UnExpectedRequestException {
        ...
        if (CollectionUtils.isNotEmpty(matchScheduledTask)) {
            ...
            for (ScheduledTask scheduledTask : matchScheduledTask) {
                ...
                if (!WtssCommonConstants.WTSS_STATUS_CODE.equals(result.get(WtssCommonConstants.WTSS_RESPONSE_CODE).toString())) {
    ...
    public void release(ScheduledReleaseRequest request) throws Exception {
        ...
        try {
            if (ScheduledTaskTypeEnum.RELATION.getCode().equals(request.getTaskType())) {
                LOGGER.info("Ready to release relation tasks");
    ...
    public void checkRuleGroupIfDependedBySchedule(RuleGroup ruleGroup) throws UnExpectedRequestException {
        if (Objects.nonNull(ruleGroup)) {
            if (scheduledFrontBackRuleDao.isExists(Arrays.asList(ruleGroup.getId()))) {
                throw new UnExpectedRequestException("This rule is dependent on relation scheduled tasks: " + ruleGroup.getId());
            }
            if (scheduledWorkflowTaskRelationDao.isExists(Arrays.asList(ruleGroup.getId()))) {
                throw new UnExpectedRequestException("This rule is dependent on publish scheduled tasks: " + ruleGroup.getId());
            }
        }
    }
