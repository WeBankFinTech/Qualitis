...
public class ScheduledWorkflowBusinessController {
    ...
    public GeneralResponse<List<ScheduledWorkflowBusinessResponse>> list(@PathParam("project_id") Long projectId) throws UnExpectedRequestException {
        List<ScheduledWorkflowBusiness> list = scheduledWorkflowBusinessService.list(projectId);
        return new GeneralResponse<>(ResponseStatusConstants.OK, "ok", list.stream().map(ScheduledWorkflowBusinessResponse::new).collect(Collectors.toList()));
    }
