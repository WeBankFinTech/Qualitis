...
public class ScheduledGetSessionUtil {
    ...
    public static String encrypt(String str, String publicKey) throws NoSuchAlgorithmException, InvalidKeySpecException, UnsupportedEncodingException, NoSuchPaddingException, BadPaddingException, IllegalBlockSizeException, InvalidKeyException {
        ...
        Cipher cipher = Cipher.getInstance(WtssCommonConstants.WTSS_RSA);
    ...
    private static String getScheduledResultByRealUser(String realUser, String clusterName) throws UnExpectedRequestException, IOException, NoSuchPaddingException, NoSuchAlgorithmException, IllegalBlockSizeException, BadPaddingException, InvalidKeyException, InvalidKeySpecException, URISyntaxException {
        ...
        if (StringUtils.isNotBlank(appId) && StringUtils.isNotBlank(appSecret)) {
            url = String.format("%s/checkin?action=login&username=%s&userpwd=&appId=%s&appSecret=%s",map.get("path").toString(),realUser, appId, appSecret);
        } else {
            String token = encrypt(map.get("loginAccount").toString(), map.get("publickey").toString());
            //username放运维用户，normalUserName放实名用户
            url = String.format("%s/checkin?action=login&username=%s&userpwd=&common_secret=%s",map.get("path").toString(),realUser, token);
        }

        logger.info("request WTSS login url: " + url);
        URI uri = new URI(url);
    ...
    private static String getScheduledResult(String operationUser, String realUser, String clusterName) throws UnExpectedRequestException, IOException, NoSuchPaddingException, NoSuchAlgorithmException, IllegalBlockSizeException, BadPaddingException, InvalidKeyException, InvalidKeySpecException, URISyntaxException {
        ...
        String url;
        if (StringUtils.isNotBlank(appId) && StringUtils.isNotBlank(appSecret)) {
            url = String.format("%s/checkin?action=login&username=%s&userpwd=&normalUserName=%s&appId=%s&appSecret=%s",map.get("path").toString(), operationUser, realUser, appId, appSecret);
        } else {
            String token = encrypt(map.get("loginAccount").toString(), map.get("publickey").toString());
            //username放运维用户，normalUserName放实名用户
            url = String.format("%s/checkin?action=login&username=%s&userpwd=&normalUserName=%s&common_secret=%s",map.get("path").toString(), operationUser, realUser, token);
        }
    ...
    public static String getSessionId(String operationUser, String realUser, String cluster) throws UnExpectedRequestException {
        ...
        if (resultMap.containsKey(WtssCommonConstants.WTSS_RES_SESSION_ID)) {
            return resultMap.get(WtssCommonConstants.WTSS_RES_SESSION_ID).toString();
        } else {
            if (resultMap.containsKey(WtssCommonConstants.WTSS_RES_ERROR)) {
                throw new UnExpectedRequestException("Error!Failed To get sessionId from Scheduled System：" + resultMap.get(WtssCommonConstants.WTSS_RES_ERROR).toString());
            } else {
