...
public interface ScheduledFrontBackRuleRepository extends JpaRepository<ScheduledFrontBackRule, Long>, JpaSpecificationExecutor<ScheduledFrontBackRule> {
    ...
    /**
     * find Scheduled Task And Rule Group
     * @param scheduledTaskIds
     * @param ruleGroupIds
     * @return
     */
    @Query(value = "select * from qualitis_scheduled_front_back_rule where scheduled_task_id in(?1) and rule_group_id in(?2)", nativeQuery = true)
    List<ScheduledFrontBackRule> findScheduledTaskAndRuleGroup(List<Long> scheduledTaskIds,List<Long> ruleGroupIds);
