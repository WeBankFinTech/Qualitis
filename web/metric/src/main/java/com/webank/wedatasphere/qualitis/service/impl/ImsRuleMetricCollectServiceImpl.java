...
public class ImsRuleMetricCollectServiceImpl implements ImsRuleMetricCollectService {
    ...
    @Autowired
    private ImsMetricHistorySchedulerDao imsMetricHistorySchedulerDao;
    ...
    @Value("${metric.collector.path.scheduler_history_update:/qualitis/outer/api/v1/imsmetric/collect/scheduler/history/update}")
    private String collectorSchedulerHistoryUpdatePath;
    ...
    private final Integer HISTORY_SCHEDULER_NOT_COLLECT = 0;
    ...
    public void createOrModifySchedulers(List<AddMetricSchedulerRequest> requests) throws UnExpectedRequestException, ParseException, IllegalAccessException {
        ...
        String collectorServerUri = buildSignatureUrl(collectorServerHost, collectorSchedulerUpdatePath);

        if (CollectionUtils.isNotEmpty(imsMetricSchedulersInDb)) {
            pushSchedulerToCollectorServer(imsMetricSchedulersInDb, collectorServerUri);
        }
        if (CollectionUtils.isNotEmpty(newImsMetricSchedulers)) {
            pushSchedulerToCollectorServer(newImsMetricSchedulers, collectorServerUri);
        }
    }

    private void pushSchedulerToCollectorServer(List<ImsMetricScheduler> imsMetricSchedulers, String collectorServerUrl) throws UnExpectedRequestException {
    ...
    private void pushHistorySchedulerToCollectorServer(ModifyMetricHistorySchedulerRequest request, String collectorServerUrl) {
        try {
            Map<String, Object> paramMap = new HashMap<>();
            paramMap.put("db_name", request.getDbName());
            paramMap.put("table_name", request.getTableName());
            paramMap.put("column_name_list", request.getColumns());
            paramMap.put("collect_status", request.getCollectStatus());
            paramMap.put("proxy_user", request.getProxyUser());
            paramMap.put("start_date", request.getStartDate());
            paramMap.put("end_date", request.getEndDate());
            paramMap.put("submit_time", request.getSubmitTime());
            GeneralResponse generalResponse = restTemplate.postForObject(collectorServerUrl, paramMap, GeneralResponse.class);
            if (!ResponseStatusConstants.OK.equals(generalResponse.getCode())) {
                LOGGER.error("Failed to create history scheduler: {}", generalResponse.getMessage());
                throw new UnExpectedRequestException("Failed to create history scheduler: " + generalResponse.getMessage());
            }
        } catch (Exception e) {
            String errMsg = String.format("Server error. db: [%s], table: [%s], exception: %s", request.getDbName(), request.getTableName(), e.getMessage());
            LOGGER.error(errMsg);
        }
...
//        通过比较，本次删除的采集配置数，将是当前分区下的全量数据，那么可同时删除该分区对应的采集频率
        ...
        if (CollectionUtils.isNotEmpty(pausedMetricSchedulers)) {
            String collectorServerUri = buildSignatureUrl(collectorServerHost, collectorSchedulerUpdatePath);
            pushSchedulerToCollectorServer(pausedMetricSchedulers, collectorServerUri);
        }
...
//       如果算子或字段被修改，则需判断改后的算子-字段，是否已存在
    ...
    public GetAllResponse<MetricCollectQueryResponse> list(MetricCollectQueryRequest request) throws UnExpectedRequestException {
        ...
        Page<ImsMetricCollectDto> responsePage = imsMetricCollectDao.queryListWithPage(request.getTemplateEnName(), request.getTemplateCnName()
                , request.getClusterName(), request.getDatabase(), request.getTable(), request.getColumn()
                , request.getPartition(), request.getCreateName(), request.getUpdateName()
                , request.getCreateStartTime(), request.getCreateEndTime(), request.getUpdateStartTime()
                , request.getUpdateEndTime(), proxyUserNames, request.getPage(), request.getSize());
...
//        formatting filter_partition condition
    ...
    @Override
    public void saveHistoryScheduler(List<ModifyMetricHistorySchedulerRequest> requests) throws UnExpectedRequestException {
        checkRequest(requests);

        String collectorServerUri = buildSignatureUrl(collectorServerHost, collectorSchedulerHistoryUpdatePath);
        for (ModifyMetricHistorySchedulerRequest request: requests) {
            request.setCollectStatus(HISTORY_SCHEDULER_NOT_COLLECT);
//        同步调度配置到采集服务
            pushHistorySchedulerToCollectorServer(request, collectorServerUri);
        }
    }

    private void checkRequest(List<ModifyMetricHistorySchedulerRequest> requests) throws UnExpectedRequestException {
        try {
            CommonChecker.checkCollections(requests, "ModifyMetricHistorySchedulerRequest");
            for (ModifyMetricHistorySchedulerRequest request: requests) {
                CommonChecker.checkString(request.getDbName(), "db_name");
                CommonChecker.checkString(request.getTableName(), "table_name");
//        是否存在未执行的记录
                List<ImsMetricHistoryScheduler> imsMetricHistorySchedulers = imsMetricHistorySchedulerDao
                        .findByCollectStatus(request.getDbName(), request.getTableName(), HISTORY_SCHEDULER_NOT_COLLECT);
                if (CollectionUtils.isNotEmpty(imsMetricHistorySchedulers)) {
                    throw new UnExpectedRequestException("An not execute history scheduler exists");
                }

//      默认最近180天
                if (StringUtils.isBlank(request.getStartDate()) || StringUtils.isBlank(request.getEndDate())) {
                    FastDateFormat fastDateFormat = FastDateFormat.getInstance(DatePattern.NORM_DATE_PATTERN, TimeZone.getTimeZone("UTC"));
                    DateTime endDateTime = DateUtil.offsetDay(new Date(), -1);
                    request.setEndDate(fastDateFormat.format(endDateTime));
                    DateTime startDateTime = DateUtil.offsetDay(new Date(), -181);
                    request.setStartDate(fastDateFormat.format(startDateTime.toJdkDate()));
                } else {
                    DateTime startDateTime = DateUtil.parse(request.getStartDate(), DatePattern.NORM_DATE_PATTERN);
                    DateTime endDateTime = DateUtil.parse(request.getEndDate(), DatePattern.NORM_DATE_PATTERN);
                    if (DateUtil.between(startDateTime.toSqlDate(), endDateTime.toSqlDate(), DateUnit.DAY, false) < 0) {
                        throw new UnExpectedRequestException("The start_date must be less than end_date");
                    }
                    if (DateUtil.between(endDateTime.toSqlDate(), new Date(), DateUnit.DAY, false) <= 0) {
                        throw new UnExpectedRequestException("The end_date must be less than today");
                    }
                }

//        库表判断采集配置表的代理用户是否唯一，否则报错
                List<String> proxyUsers = imsMetricCollectDao.queryDistinctProxyUser(request.getDbName(), request.getTableName());
                if (CollectionUtils.isEmpty(proxyUsers)) {
                    throw new UnExpectedRequestException("Proxy user doesn't exist");
                }
                if (proxyUsers.size() > 1) {
                    throw new UnExpectedRequestException("Exist different proxy user under the same db and table.");
                }

                ProxyUser proxyUser = proxyUserDao.findByProxyUserName(proxyUsers.get(0));
                Map<String, Object> userConfigMap = proxyUser.getUserConfigMap();
                if (StringUtils.isNotBlank(request.getSubmitTime())) {
                    DateTime dateTime = DateUtil.parse(request.getSubmitTime(), DatePattern.NORM_DATETIME_PATTERN);
                    if(dateTime.isBefore(new Date())) {
                        throw new UnExpectedRequestException("Submit time cannot be before now");
                    }
                    if (MapUtils.isNotEmpty(userConfigMap)) {
                        String submitTimeStr = StringUtils.split(request.getSubmitTime(), SpecCharEnum.EMPTY.getValue())[1];
                        String startTime = MapUtils.getString(userConfigMap, "collect_start_time");
                        String endTime = MapUtils.getString(userConfigMap, "collect_end_time");
                        if (StringUtils.isNotBlank(startTime) && StringUtils.isNotBlank(endTime)) {
                            Date start;
                            Date end;
                            Date submit;
                            try {
                                start = new SimpleDateFormat("HH:mm:ss").parse(startTime);
                                end = new SimpleDateFormat("HH:mm:ss").parse(endTime);
                                submit = new SimpleDateFormat("HH:mm:ss").parse(submitTimeStr);
                            } catch (ParseException e) {
                                throw new UnExpectedRequestException("Failed to format time, check your time format.");
                            }
                            if (submit.after(end) || submit.before(start)) {
                                throw new UnExpectedRequestException("The submit time exceeded time range.");
                            }
                        }
                    } else {
                        throw new UnExpectedRequestException("Proxy user " + proxyUser.getProxyUserName() + ", user_config_json error");
                    }
                } else {
                    request.setProxyUser(proxyUsers.get(0));
                }
            }
        } catch (UnExpectedRequestException e) {
...
//        }
    ...
    private String buildSignatureUrl(String host, String path) {
        ...
        return host + path + "?app_id=" + appId + "&timestamp=" + timestamp + "&nonce=" + nonce + "&signature=" + signature;
    }
