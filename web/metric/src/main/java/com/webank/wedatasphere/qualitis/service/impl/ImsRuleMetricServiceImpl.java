...
public class ImsRuleMetricServiceImpl implements ImsRuleMetricService {
    ...
    @Value("${metric.rate_limit.permits:3}")
    private Integer permits;
    @Value("${metric.rate_limit.period_seconds:3600}")
    private Integer periodSeconds;
    @Value("${metric.last_time_range_hour:1}")
    private Integer lastTimeRange;
    private Cache<String, AtomicInteger> requestCounter;
    ...
    private final DateFormat df = new SimpleDateFormat(dfStr);
    ...
    public void after() {
        ...
        requestCounter = CacheBuilder.newBuilder()
                .expireAfterWrite(periodSeconds, TimeUnit.SECONDS)
                .build();
    ...
    public GeneralResponse<GetAllMetricResponse<ImsRuleMetricQueryResponse>> getMetricDataFromOuter(ImsRuleMetricQueryRequest request) throws UnExpectedRequestException, PermissionDeniedRequestException {
        ...
        if (DateUtil.between(startDate, endDate, DateUnit.DAY) > 366) {
            throw new UnExpectedRequestException("Out of time range: 366 days.");
        }
        ...
        if (REQUEST_SOURCE.equals(request.getRequestSource())) {
            ...
            if (CollectionUtils.isEmpty(imsMetricList)) {
                LOGGER.warn("imsMetricList is empty.");
            ...
            if (CollectionUtils.isEmpty(imsMetricCollectList)) {
                LOGGER.warn("imsMetricCollectList is empty.");
//                查询采集配置是为了验证请求用户的身份，而当查询不到采集配置时则无法验证，因此按无权限异常处理
...
//        根据指标查询qualitis_metric_collect表，获取指标配置列表
    ...
    @Override
    public ImsmetricCollectDoneViewOuterResponse getMetricCollectDoneListForOuter() throws UnExpectedRequestException {
        String key = "getMetricCollectDoneListForOuter";
        checkRateLimit(key);

//        过去1个小时
        LocalDateTime endTime = LocalDateTime.now();
        LocalDateTime startTime = endTime.minusHours(lastTimeRange);
        String startTimeStr = startTime.format(DateTimeFormatter.ofPattern(dfStr));
        String endTimeStr = endTime.format(DateTimeFormatter.ofPattern(dfStr));

        ImsmetricCollectDoneViewOuterResponse response = new ImsmetricCollectDoneViewOuterResponse();
        response.setStartTime(startTimeStr);
        response.setEndTime(endTimeStr);

        List<ImsMetric> ruleMetricList = imsMetricDao.findByBetweenModifyTime(startTimeStr, endTimeStr);
        if (CollectionUtils.isEmpty(ruleMetricList)) {
            return response;
        }

//        查询采集配置
        List<Long> collectIds = ruleMetricList.stream().map(ImsMetric::getMetricCollectId).distinct().collect(Collectors.toList());
        List<ImsMetricCollect> imsMetricCollectList = imsMetricCollectDao.findByIds(collectIds);
        Map<Long, ImsMetricCollect> metricCollectMap = imsMetricCollectList.stream().collect(Collectors.toMap(ImsMetricCollect::getId, Function.identity(), (oldVal, newVal) -> oldVal));

//        查询（算子）模板
        List<Long> templateIds = imsMetricCollectList.stream().map(ImsMetricCollect::getTemplateId).distinct().collect(Collectors.toList());
        List<Template> templateList = ruleTemplateDao.findByIds(templateIds);
        Map<Long, Template> templateMap = templateList.stream().collect(Collectors.toMap(Template::getId, Function.identity(), (oldVal, newVal) -> oldVal));

        List<String> proxyUserNameList = imsMetricCollectList.stream().map(ImsMetricCollect::getProxyUser).distinct().collect(Collectors.toList());
        List<ProxyUser> proxyUserList = proxyUserDao.findByProxyUserNameList(proxyUserNameList);
        Map<String, ProxyUser> proxyUserMap = proxyUserList.stream().collect(Collectors.toMap(ProxyUser::getProxyUserName, Function.identity(), (oldVal, newVal) -> oldVal));

        List<ImsmetricCollectDoneViewOuterResponse.MetricInfo> metricInfoList = ruleMetricList.stream().map(ruleMetric -> {
            ImsmetricCollectDoneViewOuterResponse.MetricInfo metricInfo = new ImsmetricCollectDoneViewOuterResponse.MetricInfo();
            if (!metricCollectMap.containsKey(ruleMetric.getMetricCollectId())) {
                return null;
            }
            ImsMetricCollect imsMetricCollect = metricCollectMap.get(ruleMetric.getMetricCollectId());
            String proxyUserName = imsMetricCollect.getProxyUser();
            ProxyUser proxyUser = proxyUserMap.get(proxyUserName);
            metricInfo.setDepName(proxyUser == null || proxyUser.getDepartment() == null ? null : proxyUser.getDepartment().getName());
            metricInfo.setDbName(imsMetricCollect.getDbName());
            metricInfo.setTableName(imsMetricCollect.getTableName());
            metricInfo.setId(ruleMetric.getId());
            metricInfo.setPartition(imsMetricCollect.getFilter());
            metricInfo.setProxyUser(imsMetricCollect.getProxyUser());
            Template template = templateMap.get(imsMetricCollect.getTemplateId());
            metricInfo.setCalcuCnName(template.getName());
            metricInfo.setCalcuEnName(template.getEnName());
            metricInfo.setEnumValue(ScanRulesUtils.auditSenseInfoFromIdentifyValue(ruleMetric.getIdentifyValue()));
            metricInfo.setColumn(imsMetricCollect.getColumnName());
            return metricInfo;
        }).filter(Objects::nonNull).collect(Collectors.toList());
        response.setMetricInfoList(metricInfoList);

        return response;
    }

    private void checkRateLimit(String key) throws UnExpectedRequestException {
        AtomicInteger count = requestCounter.getIfPresent(key);
        if (count == null) {
            count = new AtomicInteger(0);
            requestCounter.put(key, count);
        }
        if (count.get() >= permits) {
            throw new UnExpectedRequestException("系统繁忙，请稍后再试");
        }
