...
public class Filter1AuthorizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (overseasVersionEnabled && StringUtils.isBlank(username)){
            ...
            writeRedirectHome(httpServletResponse);
            return;
        }
        ...
        if (!permitUrlList.contains(requestURI)) {
            Map<String, Object> user = HttpUtils.getUser(httpServletRequest);
            if (user == null || user.isEmpty() || StringUtils.isBlank(HttpUtils.getUserName(httpServletRequest))
